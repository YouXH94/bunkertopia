...
- 基地地图：打开 `res://scenes/base/BaseHub.tscn`，编辑 `TileMap` 节点；它已经挂好完整共享 `TileSet`，内部 layer 包含 `Ground`、`Road`、`Floor`、`Props`、`Buildings`、`Defense`、`Collision`
...
- 当前 TileSet 已接入 64x64 地表瓦片、地图建筑/废墟/防御/生产设施单图块和一个物理层；碰撞形状、Terrain Set、额外 TileMap layer 可在 Godot 编辑器中继续手动配置。
- 室内实验室套件已加入 TileSet：`bunker_entrance`、`lab_floor*`、`lab_wall*`、`supercomputer`、`supercomputer_screen_frame_00..07`、`bunker_bed`、`rusty_table`、`lab_desk`、`rusty_locker`、`metal_shelf`、`metal_chair`。
- 自然环境图集已加入 TileSet：`stone_road_atlas` 是 4x4 石头道路拼接图集，`mountain_atlas` 是 6x4 高低差山地图集（高地平台、前侧崖边、侧崖、角落、斜坡、洞口、碎石坡），`nature_atlas` 是 6x5 自然物图集，包含 3 个 2x3 大树多格图块和 12 个 1x1 灌木、花草、石块、树桩等小物；三者均由内置生图工具生成源图，再本地切齐为 TileMap atlas。
- 可手绘对象层：`PropLayer` 放废墟/容器/残骸，`BuildingPaintLayer` 放地堡/实验室/生产设施，`DefensePaintLayer` 放墙/门/陷阱/炮塔，`CollisionPaintLayer` 用来测试或单独画带碰撞的阻挡瓦片。
- 建筑、可交互点、自由建造、防守寻路仍由脚本和数据系统驱动；地面 TileMap 只负责可绘制地图底层。
- 重新生成 TileSet：`godot --headless --path . --script tools/generate_tilemap_tileset.gd`
...
python3 tools/art_pipeline/generate_lab_interior_assets.py
python3 tools/art_pipeline/generate_nature_environment_assets.py
python3 tools/art_pipeline/validate_art_assets.py
godot --headless --path . --import
godot --headless --path . --script tools/generate_tilemap_tileset.gd
```
